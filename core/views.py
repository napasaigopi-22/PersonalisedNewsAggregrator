...
def scrape(request, name):
    ...
    News = soup.find_all("div", {"class": "wp-block-gridible-grid-container gridible-row gridible-row--flex gridible-row__all-start gridible-row__md-start gridible-row__lg-start gridible-row__all-reverse-row gridible-row__md-reverse-row gridible-row__lg-standard"})
    if(News == []):
        News = soup.find_all("div", {"class": "wp-block-gridible-grid-container gridible-row gridible-row--flex gridible-row__all-start gridible-row__md-start gridible-row__lg-start gridible-row__all-reverse-row gridible-row__md-standard gridible-row__lg-standard"})
    for article in News:
        main = article.find_all("a", href=True)
        linkx = article.find("a", {"target": "_self"})
        link = linkx["href"]
        titlex = article.find("h3", {"class": "has-link-color wp-elements-aad9b6425bfbbfe81f8771ca6f420d00 wp-block-post-title has-text-color has-primary-2-color has-rocky-condensed-font-family"})
        title = titlex.text
        imgx=""
        if(article.find("img")):
            imgx = article.find("img")["src"]
        #storing extracted data to model
        new_headline = Headline()
        new_headline.title = title
        new_headline.url = link
        new_headline.image = imgx
        new_headline.average_rating = 0
        new_headline.rating_count = 0
        new_headline.save()
